...
//! Inline-test scaffold extracted from `src/handlers/mod.rs` as part
//! of the issue #650 file-architecture cleanup. Attached to the
//! parent module via `#[cfg(test)] #[path = "tests.rs"] mod tests;`.
//!
//! `use super::*` pulls in the re-exported `handlers::*` surface
//! (every handler + helper from the per-domain sub-modules). The
//! explicit imports below replace the test-only `#[cfg(test)] use`
//! lines that previously sat at the top of `mod.rs`.

// #873 — large per-test scaffolding plus a few seed helpers exceed the
// 250-line budget. Test scaffolds get the same allowance as the
// production files they pin (no need to chunk a test setup helper into
// per-step closures); tracked alongside the parent #650 follow-up.
...
// Axum extractors / types used by individual inline tests.
use axum::Json;
    ...
    let resp = app
        .oneshot(
            axum::http::Request::builder()
                ...
                .method("DELETE")
                // #870 (security-high) — DELETE is owner-scoped; the
                // authenticated caller must match `created_by`.
                .header("x-agent-id", "alice")
                .body(Body::empty())
                .unwrap(),
        )
...
/// (top-level `namespace` field, alongside `namespace_filter`). Post
/// #872 the listing is always scoped to the authenticated caller — the
/// fixture seeds both rows under the same owner that the request
/// authenticates as so the assertion still covers the projection shape.
#[tokio::test]
...
/// #872 (security-high, 2026-05-18) — listing is owner-scoped: the
/// authenticated caller (alice) only sees her own rows; bob's rows
/// must be excluded even though both rows live in the same DB. The
/// caller-supplied `?agent_id=alice` query param is honored as a
/// no-op refinement since it matches the authenticated caller.
#[tokio::test]
...
async fn h8b_notify_oversized_payload_rejected() {
    ...
    let bytes = axum::body::to_bytes(resp.into_body(), 1024 * 1024)
        ...
        .unwrap();
    let v: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
    // Issue #851: the notify endpoint now uses bad_request_opaque,
    // returning the constant sanitized envelope rather than the
    // upstream validate "exceeds max" template. Status-code invariant
    // preserved (BAD_REQUEST above); body asserted on the sanitized
    // constant. The raw "exceeds max" detail is still emitted via
