...
/// `memory_links.attest_level` column for outbound link signing,
/// v24 from I2's `memory_transcript_links` join table connecting
/// memories to their transcript provenance, and v25 from H5's
/// append-only `signed_events` audit table backing the immutable
/// attestation chain — all part of the attested-cortex epic). When
/// a DB's `schema_version` exceeds this, the binary is too old for
/// a newer DB and we surface a warning. v0.6.3.1 (PR-9h / issue
/// #487 PR #497 req #72).
