...
Steady-state curator cycle on this DB at 2026-05-17 (before any MTP
work): ≈ 100–130 s for an eligible set of ~20 memories × 2 LLM calls
each (auto_tag + contradiction adjacency). The MTP cycle (post-
restart) lands within the same band — same `cycle_duration_ms`,
same `personas_generated`, same `errors_total` distribution. See the
`_curator/reports` rows at:

```sql
SELECT created_at, json_extract(content,'$.cycle_duration_ms') AS dur_ms
FROM memories WHERE namespace='_curator/reports' ORDER BY created_at DESC LIMIT 10;
```
