...
### Agent-experience refinements (added 2026-05-04 from v0.6.3.1 OpenClaw behavioral assessment)

Three-agent unanimous P7 RoadMap signals from the [v0.6.3.1 behavioral assessment](https://alphaonedev.github.io/ai-memory-a2a-v0.6.3.1/nhi/openclaw-behavioral-v0.6.3.1/) — 52 probes, three independent OpenClaw agents (xai/grok-4.3) converged on the same friction points after running cross-session recall, multi-agent collaboration, conflicting-memory resolution, and KG reasoning tasks. Filed as issues, assigned to milestone v0.6.4, label `v0.6.4-candidate`:

23. **Auto-suggest `memory_link` during/after `memory_store`** — [#517](https://github.com/alphaonedev/ai-memory-mcp/issues/517). Manual linking was named #1 friction by all three agents in KG reasoning + multi-agent collab.
24. **Session-aware `memory_recall` defaults + auto-cue on session start** — [#518](https://github.com/alphaonedev/ai-memory-mcp/issues/518). Converts the Phase 9 organic-no-cue failure (agent confabulated bootstrap activity instead of reaching for memory) into a default success.
25. **Proactive conflict detection inside `memory_store`** — [#519](https://github.com/alphaonedev/ai-memory-mcp/issues/519). Three agents named post-hoc `memory_detect_contradiction` + manual approve/reject as too slow.
...
### Track G — Agent-experience refinements (Wed–Thu, stretch)

Three issues sourced from the v0.6.3.1 OpenClaw behavioral assessment — three independent agents converged on these as the top-3 friction points. Each is **scope-flexible**: if the v0.6.4 sprint runs hot, any subset can defer to v0.6.4.1 / v0.6.5 without losing the v0.6.4 ship date. Track them on the v0.6.4 milestone for visibility; demote on Wed standup if scope pressure is real.

#### v0.6.4-G1 — Auto-suggest `memory_link` during/after `memory_store` ([#517](https://github.com/alphaonedev/ai-memory-mcp/issues/517))
- **Goal:** extend `memory_store` MCP tool response with a `linked_candidates[]` field — top-N semantically-similar existing memories with proposed relation strings (`supersedes`, `related_to`, `contradicts`, etc.)
- **Mechanism:** HNSW similarity search inside the store handler over the same embedding the write produced; namespace + title pattern + tag overlap inform proposed relations
- **Acceptance:** unit test for similarity threshold; integration test asserts top-3 candidates returned for a write into a populated namespace; agents in P7 verbatim cite this as their #1 unmet need
- **Dep:** none (additive response field; backward compatible)
- **Files:** `src/handlers.rs` (store handler), `src/mcp/families/core.rs`, `src/db.rs::find_similar`
- **Estimate:** 3 days. Demote to v0.6.4.1 if sprint slips.

#### v0.6.4-G2 — Session-aware `memory_recall` defaults + auto-cue ([#518](https://github.com/alphaonedev/ai-memory-mcp/issues/518))
- **Goal:** two complementary additions:
  - `memory_recall --session-default` flag pulls scope (namespaces, recency, tags, tier) from `agents.defaults.recall_scope` config block
  - Session-start auto-cue: when the agent runtime opens a new session whose project context matches a persistence-relevant entry in `agents.defaults`, automatically inject `memory_recall` results into the system prompt
- **Why this matters:** Phase 9 organic-no-cue recovery was 0/1 — agent confabulated bootstrap activity instead of reaching for memory. The cued recovery was 1/1. Cue language is the gating factor; converting it to a default eliminates the failure case.
- **Acceptance:** config-driven test fixture with 3 scoped agents; integration test shows correct recall scope per agent; SDK helper documented
- **Dep:** none
- **Files:** `src/handlers.rs::recall`, `src/config.rs`, `docs/ADMIN_GUIDE.md`
- **Estimate:** 2 days. Demote to v0.6.4.1 if sprint slips.

#### v0.6.4-G3 — Proactive conflict detection inside `memory_store` ([#519](https://github.com/alphaonedev/ai-memory-mcp/issues/519))
- **Goal:** `memory_store` accepts optional `--detect-conflicts` flag (or runtime config default). When set, the substrate runs the same logic as `memory_detect_contradiction` against the proposed write versus existing memories in the same namespace; if a conflict is detected, the response includes a `conflicts[]` block with each conflicting memory + a suggested `merge_strategy` (`replace` / `link.supersedes` / `link.contradicts` / `consolidate`)
- **Why this matters:** Phase 5 trust calibration showed agents weight the right signals when conflicts ARE surfaced — but only when the agent EXPLICITLY queried + got both records. Surfacing conflicts at write time eliminates the post-hoc detect+resolve round trip.
- **Acceptance:** unit test for conflict detection logic; integration test with seeded conflict in namespace; response schema includes valid `conflicts[]` payload
- **Dep:** v0.6.4-G1 (similarity search helper) for ergonomics
- **Files:** `src/handlers.rs::store`, `src/contradiction.rs`, `src/models.rs::StoreResponse`
- **Estimate:** 3 days. Demote to v0.6.4.1 if sprint slips.

### Track G-existing — Docs + release (Fri) [renamed from G to G' to avoid collision]
