...
Tasks 5-8 (signed-events `reflection.depth_exceeded` audit row, `pre_reflect` / `post_reflect` hook events, ship-gate test suite, docs + reproducibility script) land on the same branch and roll up into this v0.7.0 entry. Tracker issue: [#655](https://github.com/alphaonedev/ai-memory-mcp/issues/655).
